...
function switchMainTab(tab) {
  ...
  ['charts','portfolio','history','risk','predictive','scanner','verdict','bot'].forEach(name => {
    ...
    if (!el) return;
    if (name === tab) {
      el.style.display = ['portfolio','predictive','bot'].includes(name) ? 'flex' : '';
      el.style.flex = '1';
...
function _botFmt(v, prefix = '', suffix = '', decimals = 2) {
  return prefix + v.toFixed(decimals) + suffix;
}
function _botUsd(v)  { return '$' + v.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ','); }
function _botSign(v) { return v >= 0 ? '+' : ''; }
function _botPct(v)  { return _botSign(v) + v.toFixed(2) + '%'; }

function botRenderKpis(k, initial) {
  const el = document.getElementById('bot-kpi-bar');
  if (!el) return;
  const retCls = k.total_return_pct >= 0 ? 'pnl-pos' : 'pnl-neg';
  const cards = [
    ['Equity',    `<span>${_botUsd(k.equity)}</span>`],
    ['Return',    `<span class="${retCls}">${_botPct(k.total_return_pct)}</span>`],
    ['Cash',      `<span>${_botUsd(k.cash)}</span>`],
    ['Win Rate',  k.total_trades > 0
      ? `<span>${k.win_rate.toFixed(1)}% <small>(${k.wins}W/${k.losses}L)</small></span>`
      : '<span>—</span>'],
    ['Max Drawdown', `<span class="${k.max_drawdown_pct > 0 ? 'pnl-neg' : ''}">-${k.max_drawdown_pct.toFixed(2)}%</span>`],
    ['Pozície',   `<span>${k.open_count} / 8</span>`],
  ];
  el.innerHTML = cards.map(([label, val]) =>
    `<div class="bot-kpi-card"><div class="bot-kpi-label">${label}</div><div class="bot-kpi-val">${val}</div></div>`
  ).join('');
}

function botRenderOpenPositions(positions) {
  const el = document.getElementById('bot-open-table');
  if (!el) return;
  if (!positions.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px 0;">Žiadne otvorené pozície.</div>';
    return;
  }
  const rows = positions.map(p => {
    ...
    const sign = p.pnl >= 0 ? '+' : '';
    return `<tr>
      <td><span class="ticker-link" onclick="openScannerTicker('${p.ticker}')">${p.ticker}</span></td>
      ...
      <td><button class="btn danger" style="padding:2px 8px;font-size:11px;" onclick="botClosePosition('${p.ticker}')">Zavri</button></td>
    </tr>`;
  }).join('');
  el.innerHTML = `<table class="tool-table" style="width:100%">
    <thead><tr><th>Ticker</th><th>Vstup</th><th>Teraz</th><th>P/L $</th><th>P/L %</th><th>Dátum</th><th></th></tr></thead>
    <tbody>${rows}</tbody>
  </table>`;
}

function botRenderHistory(trades) {
  const el = document.getElementById('bot-history-table');
  if (!el) return;
  if (!trades.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px 0;">Žiadne uzavreté obchody.</div>';
    return;
  }
  const reasonLabel = r => ({stop_loss:'Stop-loss', take_profit:'Take-profit',
    counter_signal:'Counter', manual:'Manuálne'}[r] || r);
  const rows = trades.slice(0, 40).map(t => {
    const cls  = t.pnl >= 0 ? 'pnl-pos' : 'pnl-neg';
    const sign = t.pnl >= 0 ? '+' : '';
    return `<tr>
      <td><span class="ticker-link" onclick="openScannerTicker('${t.ticker}')">${t.ticker}</span></td>
      <td style="color:var(--muted)">${t.entry_date}</td>
      <td style="color:var(--muted)">${t.exit_date}</td>
      <td>$${t.entry_price.toFixed(2)}</td>
      <td>$${t.exit_price.toFixed(2)}</td>
      <td class="${cls}">${sign}$${t.pnl.toFixed(2)}</td>
      <td class="${cls}">${sign}${t.pnl_pct.toFixed(2)}%</td>
      <td style="color:var(--muted);font-size:11px;">${reasonLabel(t.exit_reason)}</td>
    </tr>`;
  }).join('');
  el.innerHTML = `<table class="tool-table" style="width:100%">
    <thead><tr><th>Ticker</th><th>Vstup</th><th>Výstup</th><th>Cena vstup</th><th>Cena výstup</th><th>P/L $</th><th>P/L %</th><th>Dôvod</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>`;
}

function botRenderEquityCurve(curve, initial) {
  const el = document.getElementById('bot-equity-chart');
  if (!el) return;
  if (!curve || curve.length < 2) {
    el.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:40px;text-align:center;">Zatiaľ žiadne dáta equity krivky.</div>';
    return;
  }
  ...
  const minV = Math.min(initial * 0.92, ...vals);
  const maxV = Math.max(initial * 1.08, ...vals);
  const range = maxV - minV || 1;
  const n = curve.length;
  const toX = i => pad.l + (i / Math.max(n - 1, 1)) * (W - pad.l - pad.r);
  const toY = v => pad.t + (1 - (v - minV) / range) * (H - pad.t - pad.b);

  const pts = curve.map((p, i) => `${toX(i).toFixed(1)},${toY(p.equity).toFixed(1)}`).join(' ');
  const baseY = toY(initial).toFixed(1);
  const lastVal = vals[vals.length - 1];
  const lineColor = lastVal >= initial ? 'var(--up)' : 'var(--down)';

  const xIdxs = n <= 5 ? curve.map((_, i) => i) : [0, Math.floor(n / 2), n - 1];
  const xLabels = xIdxs.map(i =>
    `<text x="${toX(i).toFixed(1)}" y="${H - 4}" fill="var(--muted)" font-size="9" text-anchor="middle">${curve[i].date.slice(5)}</text>`
  ).join('');
  const yVals = [minV, initial, maxV].filter((v, i, a) => a.indexOf(v) === i);
  const yLabels = yVals.map(v =>
    `<text x="${pad.l - 4}" y="${(toY(v) + 3).toFixed(1)}" fill="var(--muted)" font-size="9" text-anchor="end">$${Math.round(v)}</text>`
  ).join('');

  el.innerHTML = `<svg width="100%" height="${H}" viewBox="0 0 ${W} ${H}">
    <line x1="${pad.l}" y1="${baseY}" x2="${W - pad.r}" y2="${baseY}"
          stroke="var(--border2)" stroke-dasharray="3,3" stroke-width="1"/>
    <polyline points="${pts}" fill="none" stroke="${lineColor}" stroke-width="2" stroke-linejoin="round"/>
    <circle cx="${toX(n-1).toFixed(1)}" cy="${toY(lastVal).toFixed(1)}" r="3" fill="${lineColor}"/>
    ${xLabels}${yLabels}
  </svg>`;
}
  ...
  try {
    ...
    if (statusEl) {
      statusEl.textContent = _botData.last_run
        ...
        : 'Bot nebol ešte spustený. Klikni ▶ Spustiť kolo.';
    }
  } catch (e) {
...
async function botRun() {
  ...
  const statusEl = document.getElementById('bot-status-msg');
  if (btn) btn.disabled = true;
  if (statusEl) statusEl.textContent = 'Spúšťam kolo bota… (môže trvať 10–30 s podľa veľkosti watchlistu)';
  try {
    const r = await fetch('/api/bot/run', { method: 'POST' });
    if (!r.ok) throw new Error(await r.text());
    const res = await r.json();
    const opened = res.actions.filter(a => a.action === 'open').length;
    const closed = res.actions.filter(a => a.action === 'close').length;
    if (statusEl) {
      const detail = res.actions.map(a =>
        a.action === 'open'
          ? `+${a.ticker} @$${a.price}`
          : `−${a.ticker} ${a.reason} ${_botPct(a.pnl_pct)}`
      ).join('  ·  ');
      statusEl.textContent = `Hotovo — skenovaných ${res.tickers_scanned}, otvorených ${opened}, uzavretých ${closed}.` +
        (detail ? `  ${detail}` : '  Žiadne zmeny.');
    }
    await botRefresh();
  } catch (e) {
    if (statusEl) statusEl.textContent = 'Chyba: ' + e.message;
...
async function botClosePosition(ticker) {
  if (!confirm(`Zavrieť pozíciu ${ticker}?`)) return;
  const statusEl = document.getElementById('bot-status-msg');
  try {
    const r = await fetch(`/api/bot/close/${encodeURIComponent(ticker)}`, { method: 'POST' });
    if (!r.ok) throw new Error(await r.text());
    const res = await r.json();
    if (statusEl) statusEl.textContent =
      `${ticker} uzavretý: ${_botSign(res.pnl)}$${res.pnl.toFixed(2)} (${_botPct(res.pnl_pct)})`;
    await botRefresh();
  } catch (e) {
    if (statusEl) statusEl.textContent = 'Chyba: ' + e.message;
  }
...
async function botReset() {
  if (!confirm('Resetovať bota? Vymaže všetky pozície a celú históriu.')) return;
  const statusEl = document.getElementById('bot-status-msg');
  try {
    const r = await fetch('/api/bot/reset', { method: 'POST' });
    if (!r.ok) throw new Error(await r.text());
    if (statusEl) statusEl.textContent = 'Bot resetovaný na počiatočný kapitál $10 000.';
    await botRefresh();
  } catch (e) {
    if (statusEl) statusEl.textContent = 'Chyba: ' + e.message;
  }
