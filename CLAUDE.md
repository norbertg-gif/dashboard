...
- **Top movers ("dynamický preset").** `GET /api/movers?account=&n=6&direction=down|up` returns the top-N stock/ETF by daily % change across watchlist (whole) + portfolio (stock/ETF only via `type`, so crypto is excluded). Daily % is `_daily_change_from_cache()` — last two daily candles from the OHLCV cache, **cache-only, no API quota**; cold symbols are skipped (warm via prefetch). Frontend `loadMovers()` (header button `📉 Top pohyby` + `Rast` checkbox for `up`) clears panels and opens 6 chart panels for the returned tickers — a one-click dynamic preset. Approximate daily reference (prev close), same caveat as daily P/L.
