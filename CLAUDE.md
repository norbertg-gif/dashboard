...
- **Verdict "Koľko kúpiť" kalkulátor.** `buildPositionSizing()` (verdict.js) je deterministický risk-based sizing pod bŕzdami: ATR14 z `daily_candles` client-side, equity z `portfolioAccountData[acct].summary._liveEquity` (fallback `etoroSummary[].equity`) SČÍTANÉ cez oba účty, `risk_per_trade_pct × equity / (atr_stop_mult × ATR14)` = počet akcií, capnuté na `dca_max_weight` (koncentračné riziko — flag `capped:true`). Fail-soft: bez equity/candles vráti `{available:false, reason}`. Žiadne nové API — všetko z UŽ načítaných dát. Prahy meniteľné cez ⚙ (`set-risk-per-trade`, `set-atr-stop-mult`), rovnaká vrstva ako DCA prahy. Interpretačná pomôcka — NIKDY nespúšťa obchod, len vypočíta.
