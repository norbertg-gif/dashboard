...
- **Verdict "Koľko kúpiť" kalkulátor.** `buildPositionSizing()` (verdict.js) je deterministický risk-based sizing pod bŕzdami: ATR14 z `daily_candles` client-side, **voľný cash** (nie equity — používateľská voľba, lebo equity zahŕňa aj kapitál uviazaný v existujúcich pozíciách) z `portfolioAccountData[acct].summary.cash` (fallback `etoroSummary[].cash`) SČÍTANÝ cez oba účty, `risk_per_trade_pct × cash / (atr_stop_mult × ATR14)` = počet akcií, capnuté na `dca_max_weight` % cash (nie % equity — flag `capped:true`). Fail-soft: bez cash/candles vráti `{available:false, reason}` (equity bez cash NESTAČÍ — plne investovaný účet je zámerne "nedostupný", nie fallback na equity). Žiadne nové API — všetko z UŽ načítaných dát. Prahy meniteľné cez ⚙ (`set-risk-per-trade`, `set-atr-stop-mult`), rovnaká vrstva ako DCA prahy. Interpretačná pomôcka — NIKDY nespúšťa obchod, len vypočíta.
