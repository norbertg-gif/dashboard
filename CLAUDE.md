...
- **Portfolio cache (positions/orders) TTL = 30 min, RAM backed by disk.** `POSITIONS_CACHE_TTL = 1800` (2026-07-09, raised from 120s) — pri štýle max pár obchodov týždenne sa zoznam pozícií/objednávok mení zriedka; live ceny a P/L idú aj tak nezávisle cez WS tick vrstvu (`estimatePositionLivePnl`), takže dlhší TTL neznamená zastarané čísla na obrazovke. `get_portfolio` (`/api/etoro/portfolio`) ukladá processed shape (`positions/summary/mirrors/orders`) na disk (`cache/portfolio/processed_{account}.gz`, oddelené od legacy raw-data cache pod `/api/etoro/positions`, ktorý je dnes fail-soft no-op) — cold start (Render free tier po nečinnosti) tak servíruje posledný známy stav z disku okamžite namiesto blokovania na eToro round-trip. Na chybu z eToro proxy padá späť na (aj staršiu) cache namiesto 502 (`stale: true`) — stale-while-erroring je zámerné. `refresh=1` úplne obíde cache. Frontend zrkadlí rovnaký TTL (`ETORO_POSITIONS_TTL_MS` v `live.js`, 30 min) pre `etoroPositionsAll`/`etoroOrdersAll` (chart panely + Analytika) — `positionsStale()` kontroluje podľa `etoroPositionsFetchedAt`, NIE podľa `.length` (účet bez otvorených pozícií by inak vyzeral "stale" navždy). `loadPositionsForAccount()` má in-flight promise dedup — keď viac panelov naraz zistí "stale" (napr. Load All), zdieľajú jeden fetch namiesto N súbežných. Manuálne obnovenie: Portfólio ⟳ (`loadPortData(pid, true)`) posiela `refresh=1` a zároveň zapíše čerstvé dáta aj do `etoroPositionsAll`/`etoroOrdersAll`, aby sa nerozišli s tým, čo vidia grafy.
