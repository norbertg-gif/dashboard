...
- **DIP universe scanner** — legacy endpoints remain `/api/scanner/nasdaq/*`, but an imported DIP ranking XLSX is now the primary scanned universe (capped by `SCANNER_DIP_UNIVERSE_MAX`, default 300). Nasdaq-100 is only the fallback when no DIP import exists. Both universes append missing held Stock/ETF symbols from `_get_portfolio_holdings()` (RAM/disk cache only, no eToro calls; empty/failing cache preserves the original universe), normalized/validated and sorted after the DIP ranking; `SCANNER_DIP_UNIVERSE_MAX` caps only the DIP slice, never held additions. This gives held positions outside the import scanner coverage for AI export (`dip_score`, `daily_state`, `weekly_state` and candle-derived fields). The display label gains ` + portfólio` only when holdings add symbols; keys stay `dip_import` / `nasdaq100`, and `_dip_universe` / `top_ranked_not_selected` remain strictly the imported ranking. This matters because the XLSX ranking can include NYSE/non-Nasdaq stocks and scanning Nasdaq on top of it created avoidable timeouts. The scanner uses a bounded worker queue, so waiting tickers are not timed out before they actually start. The result table keeps every successfully scanned imported ticker with `TOTAL >= 85` (`DIP_SCANNER_VISIBILITY_THRESHOLD`) even without a fresh technical signal; lower-ranked rows remain signal-only. Therefore `recent_signal` is nullable in cached scanner results: always use `(row.get("recent_signal") or {})`, including sort keys and crossover counters. HTML/bookmarklet import is intentionally disabled; legacy endpoints return 410. Large KPI cards were replaced by a compact status line.
...
- **Farebné štítky tickerov (`GET/POST /api/portfolio/tags`).** Ručná vizuálna
  pomôcka v Portfóliu: používateľ si definuje štítky (názov + farba, max 12) a
  priradí JEDEN na ticker; symbolová bunka sa prefarbí a názov štítku je v
  `title`. Ukladá sa do `DATA_ROOT/ticker_tags.json` (gitignored, atomický zápis
  pod zámkom, vzor `position_classes.json`). Definície sa zapisujú ako celý
  zoznam, priradenia čiastočne (`null` maže jedno). **Zmazanie štítku v tom
  istom zápise vyčistí aj jeho priradenia** a GET filtruje odkazy na neexistujúce
  id — inak by ostala bunka zafarbená bez vysvetlenia. Priradenie je
  OPTIMISTICKÉ (`portfolioTagAssignSeq` drží poradie): POST vie čakať v rade za
  načítavaním grafov — nameraných 9 s — a pôvodná verzia pod globálnym
  `portfolioTagsBusy` po celý ten čas ticho zahadzovala ďalšie kliky, takže to
  pôsobilo ako rozbitá funkcia. Pri chybe sa zmena vráti späť.
  **Je to čisto prezentačná vrstva** — NEVSTUPUJE do C1–C4, DIP, DCA, BUILD,
  scanner tieru, Verdiktu ani AI exportu, a je zámerne oddelená od
  `position_class` (tá riadi cieľové váhy a kapitál, toto je pracovná pozornosť).
  Nie je to náhrada za zrušenú „tézu k pozícii" — tá bola voľný text na KAŽDÝ
  trade; toto je farba na ticker, nastavená jedným pravým klikom.
...
- **Portfolio cache (positions/orders) TTL = 24h, RAM backed by disk.** `POSITIONS_CACHE_TTL = 86400` — zoznam pozícií/objednávok sa mení zriedka; live ceny a P/L idú nezávisle cez WS. `get_portfolio` (`/api/etoro/portfolio`) je jediný zdroj snapshotu a ukladá processed shape (`positions/summary/mirrors/orders`) do RAM aj `cache/portfolio/processed_{account}.gz`. Kompatibilný `/api/etoro/positions` už nevykonáva druhý eToro fetch ani nemá vlastnú cache; iba preloží tento snapshot na stock/ETF legacy tvar. Na chybu proxy sa vracia aj staršia cache (`stale: true`); `refresh=1` ju obíde. Frontend zrkadlí 24h TTL cez `ETORO_POSITIONS_TTL_MS` iba pre čerstvé odpovede: `loadPositionsForAccount` ponechá stale snapshot zobrazený a označený v `etoroPositionsStale`, ale neobnoví jeho timestamp, takže ďalšie otvorenie grafu ho skúsi načítať znovu. Používa in-flight promise dedup a manuálny Portfólio refresh zosúladí aj chart/Analytika pozičnú cache.
...
- **`stale: true` z backendu sa ticho ignorovalo.** Pri výpadku eToro proxy `get_portfolio` vráti 200 so starými dátami a `stale: true`, ale `loadPortData` aj samostatná chart/Analytika cesta `loadPositionsForAccount` ich mohli potvrdiť ako čerstvé (reset `etoroPositionsFetchedAt`, ďalšie 24h okno). Fix: ani cesta nestampuje stale odpoveď; Portfólio ukáže "⚠ zastarané dáta" a chart/Analytika cache ju drží označenú v `etoroPositionsStale`, aby sa pri ďalšej príležitosti hneď retryla.
