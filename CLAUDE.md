...
- **Basic/Advanced UI mode.** Header button `ui-mode-btn` stores `td_ui_mode` (`advanced` default to avoid surprise after deploy). Basic is only a visual disclosure layer: CSS hides `.advanced-only`, calculations/data stay identical. Portfolio correlation map is not rendered/fetched at all in Basic; DCA stays available but opens collapsed on fresh page load.
