...
- **Top movers ("dynamický preset").** `GET /api/movers?account=&n=6&direction=down|up` returns the top-N stock/ETF by daily % change across watchlist (whole) + portfolio (stock/ETF only via `type`, so crypto is excluded). Daily % is `_daily_change_from_cache()` — last two daily candles from the OHLCV cache, **cache-only, no API quota**; cold symbols are skipped (warm via prefetch). Frontend `loadMovers()` (header button `📉 Top pohyby` + `Rast` checkbox for `up`) requests `n = cols × 2` (STĹPCE select → 2 rows: 3 cols→6, 4 cols→8), clears panels and opens that many chart panels for the returned tickers — a one-click dynamic preset. Approximate daily reference (prev close), same caveat as daily P/L.
