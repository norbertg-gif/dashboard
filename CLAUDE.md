...
- **Basic/Advanced UI mode.** Header button `ui-mode-btn` stores `td_ui_mode` (`advanced` default to avoid surprise after deploy). Basic is only a visual disclosure layer: CSS hides `.advanced-only` and shows `.basic-only`, calculations/data stay identical. Portfolio correlation map is not rendered/fetched at all in Basic; DCA stays available but opens collapsed on fresh page load. Scanner Basic hides market raw detail/import diagnostics/raw counts/error diagnostics and removes the old copy/export block entirely; watchlist/eToro radar defaults to 3 cards. Analytika Basic hides signal analytics, 90D+ validation detail, model forecast/backtesting/hit-rate/weights/entry-zone cards, detailed chart-pattern filter rows, and the History tab.
...
- **Portfolio cache (positions/orders) TTL = 24h, RAM backed by disk.** `POSITIONS_CACHE_TTL = 86400` (2026-07-09, raised from 120s) — pri štýle max pár obchodov týždenne sa zoznam pozícií/objednávok mení zriedka; live ceny a P/L idú aj tak nezávisle cez WS tick vrstvu (`estimatePositionLivePnl`), takže dlhší TTL neznamená zastarané čísla na obrazovke. `get_portfolio` (`/api/etoro/portfolio`) ukladá processed shape (`positions/summary/mirrors/orders`) na disk (`cache/portfolio/processed_{account}.gz`, oddelené od legacy raw-data cache pod `/api/etoro/positions`, ktorý je dnes fail-soft no-op) — cold start (Render free tier po nečinnosti) tak servíruje posledný známy stav z disku okamžite namiesto blokovania na eToro round-trip. Na chybu z eToro proxy padá späť na (aj staršiu) cache namiesto 502 (`stale: true`) — stale-while-erroring je zámerné. `refresh=1` úplne obíde cache. Frontend zrkadlí rovnaký TTL (`ETORO_POSITIONS_TTL_MS` v `live.js`, 24h) pre `etoroPositionsAll`/`etoroOrdersAll` (chart panely + Analytika) — `positionsStale()` kontroluje podľa `etoroPositionsFetchedAt`, NIE podľa `.length` (účet bez otvorených pozícií by inak vyzeral "stale" navždy). `loadPositionsForAccount()` má in-flight promise dedup — keď viac panelov naraz zistí "stale" (napr. Load All), zdieľajú jeden fetch namiesto N súbežných. Manuálne obnovenie: Portfólio ⟳ (`loadPortData(pid, true)`) posiela `refresh=1` a zároveň zapíše čerstvé dáta aj do `etoroPositionsAll`/`etoroOrdersAll`, aby sa nerozišli s tým, čo vidia grafy.
...
- **`_positions_cache` kolízia (kritické).** Legacy `/api/etoro/positions` (frontendom nevolaný od 5115f1f, ale priamo dostupný) zdieľal `_positions_cache[account]` s `/api/etoro/portfolio` a písal iný tvar dát (len akcie/ETF, žiadne `orders`/`mirrors`). Jedno volanie by pri 24h TTL "otrávilo" Portfólio, čakajúce objednávky aj žlté čiary v grafe na celý deň. Fix: vlastná `_legacy_positions_cache`.
...
- **`GET /api/chart` obchádzal zdieľanú `_yf_download_cached`** — dva priame `yf.download` volania (weekly + daily) pri KAŽDOM otvorení Analytiky, žiadna cache. Fix: presmerované na `_yf_download_cached(..., prefer_massive=False, munge_dots=False)` (rovnaké správanie, len cachované 30 min).
...
Neopravené/vedomé rozhodnutia z auditu (nižšia priorita, väčší zásah): `run_backtest` sa v `/api/chart` počíta 2× bez cache (kandidát na `_MODEL_CACHE` reuse), `get_public_portfolio` má vlastnú (staršiu, mierne odlišnú) summary logiku, niekoľko potvrdených dead-code funkcií vo frontende (napr. celý "Ceny/Rates" tab je nedosiahnuteľný — `#main-rates` v HTML neexistuje) — ponechané, kým sa nepotvrdí zámer.
