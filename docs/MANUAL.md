...
| **📉 Top pohyby** | „Dynamický preset" — zatvorí existujúce grafy a otvorí 6 grafov titulov s najväčším denným pohybom (len akcie/ETF z watchlistu + portfólia, crypto sa ignoruje). Default sú najväčšie **poklesy**; checkbox **Rast** prepne na najväčšie **rasty** (tlačidlo sa zmení na 📈). Denný % sa berie z OHLCV cache, takže nesťahuje nové dáta — tituly bez zahriatej cache sa preskočia. Denný pohyb je približný (referenčný včerajší close), rovnaký princíp ako denné P/L. |
