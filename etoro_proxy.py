...
Lokálne: python etoro_proxy.py
Na Render: štartuje ako background thread z trading_backend.py
"""
...
# === ÚČTY — kľúče z env variables ===
# Lokálne: nastav v start.bat alebo .env
# Render:  nastav v Render Environment Variables
ACCOUNT1 = {
    "name":     "Marvin",
...
def start_proxy_thread():
    """Spustí proxy ako daemon thread — volá sa z trading_backend.py."""
...
if __name__ == "__main__":
    ...
    for k, v in ACCOUNTS.items():
        ok = bool(v['api_key'])
        print(f"  Ucet {k}: {v['name']} - {'OK' if ok else 'KLUCE CHYBAJU (nastav env vars)'}")
